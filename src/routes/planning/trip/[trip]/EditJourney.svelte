    ...
    <div class="bg-neutral-900 w-[90%] max-w-[800px] h-[90%] overflow-y-scroll rounded-md p-4 ml-3 pl-6 z-40 cursor-default border-[1px] border-neutral-700 flex flex-col">
        ...
        <div class="flex gap-2 mt-4 border-[1px] border-neutral-700 rounded-md p-2 pb-3 sm:flex-row flex-col">
            <div class="w-full">
                ...
                <PromptField ds="{locations}" on:select={selectFrom} bind:value={from} ver="loc"/>
            </div>
            <div class="w-full">
                <h3 class="text-neutral-300 italic text-left mb-1">To</h3>
                <PromptField ds="{locations}" on:select={selectTo} bind:value={to} ver="loc"/>
            </div>
        </div>
...
<script>
import{createEventDispatcher,onMount}from"svelte";const dispatch=createEventDispatcher();export let day;export let journey;import PromptField from"../../../../lib/components/PromptField.svelte";import CustomAlert from"../../../../lib/components/Alert.svelte";import{writable}from"svelte/store";let from=journey.from,to=journey.to,arrival=journey.arrival,departure=journey.departure,service=journey.service,operator=journey.operator,description=journey.description,code=journey.code,alrtMode=writable("err"),alrtTxt=writable(""),alrtAct=writable(!1);function close(o){dispatch("message",{text:"close"})}function submit(o){dispatch("message",{text:o})}let locations=[];try{const o=localStorage.getItem("locations");locations=JSON.parse(o)||[]}catch(o){}function selectFrom(o){from=o.detail.text,console.log(from,to)}function selectTo(o){to=o.detail.text,console.log(from,to)}function cLS(o){if(localStorage.getItem("locations")){locations.find((e=>e.toLowerCase()===o.toLowerCase()))?console.log("Location already exists"):(locations.push(o),localStorage.setItem("locations",JSON.stringify(locations)),console.log("Location added"))}else locations.push(o),localStorage.setItem("locations",JSON.stringify(locations)),console.log("Location added")}function addJourneyConfirm(){if(null==from||null==to||null==arrival||null==departure)return alrtTxt.set("Please fill in all fields"),alrtAct.set(!0),console.log("Please fill in all fields"),void console.log(from,to,arrival,departure);if(confirm("Please confirm you would like to edit this journey.")){let o={day:day,journey:{code:code,from:from,to:to,arrival:arrival,departure:departure,service:service,operator:operator,description:description}};console.log("JI: ",o),submit(o),cLS(from),cLS(to)}}
</script>
